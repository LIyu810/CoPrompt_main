...
parser.add_argument("--model_name_or_path", default='/workspace/CoPrompt-main/plm/chinese-roberta-wwm-ext')
# parser.add_argument("--model_name_or_path", default='hfl/chinese-roberta-wwm-ext')
parser.add_argument("--dataset_name",default="chinese_dialogue_full")
...
def split_label_words(tokenizer, label_list):
    ...
    for label in label_list:
        if label == 'no_relation' or label == "unanswerable":
...
model_name_or_path0 = os.path.split(model_name_or_path)[-1]
with open(f"./dataset/{model_name_or_path0}_{dataset_name}.pt", "wb") as file:
    torch.save(t, file)