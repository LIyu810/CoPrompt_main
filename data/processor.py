...
class relossProcessor(DataProcessor): #bert_s
    ...
    def get_labels(self):
        """See base class."""
        return [str(x) for x in range(30)]      # 36 --> 30
...
class bertProcessor(DataProcessor): #bert_s
    def __init__(self, data_path="data", use_prompt=False, label_len=30):   # 36 --> 30
        ...
        for sid in range(3):
            ...
            with open(data_path + "/"+["train.json", "dev.json", "test.json"][sid], "r", encoding="utf8") as f:
                data = json.load(f)
            sample_idx = 0
            for i in range(len(data)):      # 遍历dialogue
                for j in range(len(data[i][1])): # n个关系对
                    rid = [] # 关系使用一个多标签label来表示
                    # for z in range(21128):
                    #     rid.append(0)
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            guid = "%s-%s" % (set_type, i)
            # print(data[i][0], data[i][1], data[i][3], data[i][2])
            # print(asdasda)
