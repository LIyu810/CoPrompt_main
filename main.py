...
def _setup_parser():
    ...
    # parser.add_argument("--litmodel_class", type=str, default="transformer")
    parser.add_argument("--litmodel_class", type=str, default="transformer.DialogueLitModel")
    ...
    # litmodel_class = lit_models.transformer.DialogueLitModel
...
def _get_relation_embedding(data):
    ...
    #! hard coded
    relation_embedding = [[] for _ in range(30)]
...
def main():
    ...
    # print(model)
    # print(asdasd)
    data = data_class(args, model)                          # ---> dialogue
    ...
    lit_model = litmodel_class(args=args, model=model, tokenizer=data.tokenizer)      # ---> transformer
    ...
    # init callbacks
    early_callback = pl.callbacks.EarlyStopping(monitor="Eval/f1", mode="max", patience=5,check_on_train_epoch_end=False)   # early_stoping
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor="Eval/f1", mode="max",
        filename='{epoch}-{Eval/f1:.4f}',
        dirpath="/workspace/CoPrompt-main/output/output_full_0629",
    ...
    trainer.fit(lit_model, datamodule=data)      # 训练
    ...
    config = vars(args)
    config["path"] = path
    if not os.path.exists(os.path.join(os.path.join("config", day_name))):
        os.makedirs(os.path.join(os.path.join("config", day_name)))
    ...
    step2_model_checkpoint = pl.callbacks.ModelCheckpoint(monitor="Eval/f1", mode="max",    # Eval/loss
